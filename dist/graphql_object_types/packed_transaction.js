...
export const packed_transaction_fields = {
    ...
    required_keys: {
        ...
        async resolve({ available_keys, transaction }, args, getContext, info) {
            const { network } = getContext({ available_keys, transaction }, args, info);
            if (available_keys?.length) {
                const keys = await Promise.all(available_keys);
                const req = await fetch(network.rpc_url + "/v1/chain/get_required_keys", {
                    method: "POST",
                    body: JSON.stringify({
                        transaction: {
                            ...transaction,
...
//# sourceMappingURL=packed_transaction.js.map