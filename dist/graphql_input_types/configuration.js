...
const configuration_type = new GraphQLInputObjectType({
    ...
    description: "Configuration input to control various aspects of the transaction.",
    fields: () => ({
        blocksBehind: {
        ...
        max_net_usage_words: {
            description: "Maximum NET bandwidth usage a transaction can consume, when set to 0 there is no limit",
            type: GraphQLInt,
            defaultValue: configuration_default_value.max_net_usage_words
        },
        max_cpu_usage_ms: {
            description: "Maximum CPU bandwidth usage a transaction can consume, when set to 0 there is no limit",
            type: GraphQLInt,
