...
interface RamMarketRow {
  supply: string; // e.g. "1000000.0000 RAM"
  base: { balance: string }; // e.g. { balance: "1000.0000 SYS" }
  quote: { balance: string }; // e.g. { balance: "500000.0000 RAM" }
}

interface RamMarketResponse {
  rows: RamMarketRow[];
  error?: any;
  message?: string;
}

const RAM_quote_type = new GraphQLObjectType<string>({
...
export const get_ram_price: GraphQLFieldConfig<unknown, any, GetRamPriceArgs> =
  {
    ...
    async resolve(root, { quantity = "1" }, context: Context, info) {
      ...
      const data: RamMarketResponse = await response.json();
